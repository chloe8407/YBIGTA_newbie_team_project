...
class IMDbProcessor(BaseDataProcessor):
    """
    IMDb 리뷰 데이터를 위한 전처리 및 파생 변수 생성 클래스입니다.
    """
    site_name = "imdb"

    def run(self) -> None:
        """
        IMDb 리뷰 데이터에 대한 전체 전처리 파이프라인을 실행합니다.
    ...
    def nltk_install(self) -> None:
        """
        필요한 NLTK 리소스(stopwords, wordnet 등)를 확인하고 없으면 다운로드합니다.
        """
    ...
    def clean_text(self, text: str) -> str:
        ...
        소문자 변환, 특수문자 제거, 불용어 제거, 어간 추출(Lemmatization)을 수행합니다.
    ...
    def preprocess(self):
        """
        원본 데이터를 로드하여 결측치 처리, 단어 수 필터링, 영어 리뷰 추출, 텍스트 정제 및 날짜 파싱을 수행합니다.
        """
        self.nltk_install()
    ...
    def feature_engineering(self):
        ...
        월 단위 날짜 정보 및 정제된 단어 수를 추가하고 주관성 점수를 계산합니다.
        """
        if self.df is None:
    ...
    def add_subjectivity_score(self):
        ...
        # TF-IDF 초기화
        # L2 정규화를 비활성화(norm=None)하여 단어 빈도와 중요도의 크기를 유지합니다.
        ...
        try:
            ...
            # 피처 순서에 맞춰 가중치 벡터 생성
            # 사전에 있는 단어는 해당 가중치를 사용하고, 없는 단어는 배경값(0.5)을 부여합니다.
            weights = []
            ...
            # 점수 계산: TF-IDF 행렬과 가중치 벡터의 내적
            raw_scores = tfidf_matrix.dot(weights)
            
            # 리뷰 길이에 따른 편향을 방지하기 위해 단어 수로 나눕니다.
